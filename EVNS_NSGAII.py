...
initial_t = time.time()
from MDR_makespan import population_number_mks, iteration, diff_best, Population_mks
from MDR_energy import population_number_ene, iteration_energy, energy, diff_dict, Population_ene

final_t = time.time()

print("Initialization time", final_t - initial_t)
...
gen = 0
# Used to store the results of each generation
result_time_list = []
result_energy_list = []
# These store the energy of the best makespan and the makespan of the best energy
...
for gennum in range(generation_num):
    ...
    result_sch_energy = schedule_result_total[index_pop_energy]
...
total_time = final_t - initial_t + round(current_time - old_time, 2)
print("Total time:", total_time, "s")

gantt(result_sch_makespan)
gantt(result_sch_energy)
...
with pd.ExcelWriter('Graph Excel/Iterations_EVNS_NSGAII.xlsx', engine='xlsxwriter') as writer:
