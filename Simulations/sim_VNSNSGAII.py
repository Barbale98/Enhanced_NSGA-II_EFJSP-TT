...
def run_simulation(time_max, filename):
    # Parameters
    Pop_size = 100
    ...
    for gennum in range(generation_num):
        ...
        if gen in range(89, generation_num, 10):
            ...
            for a in VNS_individuals:
                for i in range(10):
                    ...
                    if min(energy_total_else_local) < min_energy:
                        min_energy = round(min(energy_total_else_local), 2)
                ...
                if current_time - initial_t > time_max:
                    break
        ...
        print("The running time is " + str(round(current_time - initial_t, 2)) + "s")
        if current_time - initial_t > time_max:
            break

    pareto_data = pareto_front(data_gen_store[-1])
    return round(current_time - initial_t, 2), min_makespan, min_energy, iteration_energy, iteration, pareto_data,len(pareto_data),gen
