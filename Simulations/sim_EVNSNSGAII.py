...
def run_simulation(filename):
    ...
    class GA():
        ...
        def arranged(self, M_arranged, current_machine, P_arranged, current_product, ta, current_op_time, All_arranged,
                     key,
                     a):
        ...
        def energy_calculation(self, ALL_arranged, makespan, AGV_scheduling):
            ...
            # machine in idle
            for machine in M:
                idle_energy += (makespan - (total_production_time_machine.get(machine) or 0)) * IDLE_power[
                    machine - 1] / 60
            # agv energy
        ...
        def crowding_distance_sort(self, last_front):
            ...
            for obj_index in range(2):
                ...
                for i in range(1, num_individuals - 1):
                    distance = last_front[sorted_indices[i + 1]][obj_index] - last_front[sorted_indices[i - 1]][
                        obj_index]
            ...
            while len(selected_individuals) < min(self.Pop_size, len(time1)):
                ...
                if len(fronts[current_front]) + len(selected_individuals) <= min(self.Pop_size, len(time1)):
                    ...
                    selected_individuals.extend(
                        sorted_last_front[:(min(self.Pop_size, len(time1)) - len(selected_individuals))])
        ...
        def swap_os(self, os_to_swap, job1, operation1, job2, operation2):
            occurrence_count = 0
            # Loop through the jobs_vector to find the index of the desired occurrence
            for index, job in enumerate(os_to_swap):
                if job == job1:
                    occurrence_count += 1
            ...
            for index, job in enumerate(os_to_swap):
                if job == job2:
                    ...
                    if occurrence_count == operation2:
                        index2 = index
                        break  # Stop searching once we find the desired occurrence
            temp = os_to_swap[index1]
            os_to_swap[index1] = os_to_swap[index2]
            os_to_swap[index2] = temp
            return os_to_swap
            ...
            while var is True:
                if random_operation in [2, len(O_ij[job_critic])]:
                    ...
                    if previous_key is not None:
                        if schedule[2][(job_critic, random_operation, machine_critic)][0] == schedule[2][previous_key][
                            1] + TRT[previous_key[2], machine_critic]:
                            random_operation = random.choice([2, len(O_ij[job_critic])])
                            for key in schedule[2].keys():
                                if key[0] == job_critic and key[1] == random_operation:
                                    machine_critic = key[2]  # Return the machine part of the key
    ...
    def gantt(result_sch):
        ...
        for key in ALL.keys():
            ...
            ax.barh(key[2], width=ALL[key][1] - ALL[key][0], height=0.6, left=ALL[key][0], color=color,
                    edgecolor='black',
                    linewidth=0.3)
        ...
        for i, (t_inizio, duration, prodotto, mac_pre, mac_post, ta) in enumerate(result_sch[3]):
            if mac_pre != 0 and mac_post != 0:
                ax.barh(mac_pre + 0.4, duration, left=t_inizio, height=0.2, color='orange',
                        edgecolor='black')
                ax.text(t_inizio + duration / 2, mac_pre + 0.4, str(prodotto) + str(mac_pre) + str(mac_post),
                        ha='center',
    ...
    def pareto_front(gen_result):
        ...
        for i, individual in enumerate(gen_result):
            if individuals_dominate[i] == 0:
                ...
                Energy.append(round(individual[1],2))
    ...
    from sim_DR_improved import compute_best_indices_mks
    Population_mks, population_number_mks, iteration, diff_best = compute_best_indices_mks(filename)
    from sim_DR_improved import compute_best_indices_ene
    Population_ene, population_number_ene, iteration_energy, diff_dict, energy = compute_best_indices_ene(filename)


    for ind in Population_ene:
        Pop_list.append(ind)
    for ind in Population_mks:
        Pop_list.append(ind)

    Pop_matrix = np.array(Pop_list)

    # Operation Insertion Method
    # O_M_T_total dictionary with sorted (job,assignment,machine):processing times elements, like in OS
    ...
    for order in range(len(O_M_T_total)):
        ...
        if count < len(O_M_T_total) / 2:
            if energy_schedule <= min(energy_total):
                iteration_energy[order][1] = energy_schedule

        count += 1

    select_index, num_pareto = Pop.fitness(makespan_total, energy_total)
    Pop_list = []

    for a in select_index[:40]:
        Pop_list.append(Pop_matrix[a])

    for gennum in range(generation_num):
        gen += 1
        if gennum == 0:
            # randomly generate initial population with different probabilities
            # store individuals in pop list

            for i in range(Pop_size - population_number_mks - population_number_ene):
            ...
            for order in range(
                    len(O_M_T_total)):  # For each chromosome, find the minimum value of the entire population
                key1 = list(O_M_T_total[order].keys())
                value1 = list(O_M_T_total[order].values())
                schedule_result = Pop.Operation_insert(key1, value1)
            ...
            print("Pareto solutions:", num_pareto)
        ...
        #index_pop_energy = energy_total.index(min(energy_total))
        ...
        print("*")
        print('Gen:', gen)
        print('Makespan: Min:', min_makespan, 'Max:', max_makespan)
        print('Energy: Min:', min_energy, 'Max:', max_energy)
        #print('Best Makespan:', min_makespan, "Best makespan energy:",energy_total[makespan_total.index(min(makespan_total))])
        #print('Best Energy:', min_energy,'Best energy makespan:', makespan_total[energy_total.index(min(energy_total))])
        ...
        if gen in range(19, generation_num, 10):
            ...
            for a in VNS_individuals:
                for i in range(10):
                    ...
                    select_index_local, num_pareto_local = Pop.fitness(makespan_total_else_local,
                                                                       energy_total_else_local)
    ...
    index_pop = makespan_total.index(min(makespan_total))
    result_sch_makespan = schedule_result_total[index_pop]
    gantt(result_sch_makespan)
    return round(current_time - initial_t, 2), min_makespan, min_energy, iteration_energy, iteration, pareto_data, len(pareto_data)

# This allows the script to be run directly via command line or imported and run from another script
